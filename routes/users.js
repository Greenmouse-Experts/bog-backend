...
// @route  api/signup
router
  .route('/user/check-name')
  .post(nameValidation(), validate, UserController.findName);

router
  .route('/user/check-phone')
  .post(phoneValidation(), validate, UserController.findPhone);
 ...
 */
router.route('/user/facebook/login').post(
...
router
  ...
  .post([googleLoginValidation(), validate], UserController.googleSignin);
