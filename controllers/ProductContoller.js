  ...
  sequelize.transaction(async (t) => {
    try {
      ...
      if (!Array.isArray(productsToCheck) || productsToCheck.length === 0) {
        return res.status(400).send({ status: false, message: 'No products selected to check availability' });
      }
      ...
      const unavailableProducts = productsToCheck.filter(product => !availableProductsMap[product.id]);
      ...
      return res.send({ success: true, message: 'All products are available for checkout' });
